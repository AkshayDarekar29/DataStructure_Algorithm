...
public class RatInMaze {
	...
	private void solveMaze(int[][] matrix, int x, int y,int[] xMove, int[] yMove, ArrayList<String> list, ArrayList<String> res) {
		if(x == matrix.length-1 && y == matrix[0].length-1){
			...
			for (int i = 0; i < 4; i++) {
				...
				if(isSafe(matrix, newX, newY)){
					...
					matrix[newX][newY] = 0; //this is important
					solveMaze(matrix, newX, newY, xMove, yMove, list, res);
					matrix[newX][newY] = 1;
